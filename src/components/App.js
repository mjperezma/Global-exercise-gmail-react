...
const App = () => {
  ...
  const renderEmailDetail = (props) => {
    ...
    if (selectedEmail !== undefined) {
      return <EmailReader id={selectedEmail.id} fromName={selectedEmail.fromName} fromEmail={selectedEmail.fromEmail} subject={selectedEmail.subject} body={selectedEmail.body} deleted={selectedEmail.deleted} handleReadEmail={handleReadEmail} handleDeleteEmail={handleDeleteEmail} />;
    } else {
  ...
  return (
    <>
      <Header textFilter={textFilter} showInbox={showInbox} handleInboxFilter={handleInboxFilter} handleDeleteFilter={handleDeleteFilter} handleTextFilter={handleTextFilter} />

      <Switch>
        <Route path='/email/:emailId' render={renderEmailDetail} />
        <Route path='/'>
          <EmailTable emails={filterEmails(emails, textFilter, showInbox)} handleDeleteEmail={handleDeleteEmail} />
        </Route>
      </Switch>
